...
class BaseJsonLogger:
    """This is the base class that returns __dict__ of its own it also returns the dicts of objects in the attributes
    that are list instances.
    """
...
class Frame(BaseJsonLogger):
    ...
    def __init__(self, frame_id: int, timestamp: float | None = None):
    ...
    def add_bbox(self, bbox_id: int, top: int, left: int, width: int, height: int):
        ...
        if bbox_id not in bboxes_ids:
            ...
            raise ValueError(f"Frame with id: {self.frame_id} already has a Bbox with id: {bbox_id}")
    ...
    def add_label_to_bbox(self, bbox_id: int, category: str, confidence: float):
        ...
        if bbox_id in bboxes.keys():
            ...
            raise ValueError(f"the bbox with id: {bbox_id} does not exists!")


class BboxToJsonLogger(BaseJsonLogger):
    """ُ This module is designed to automate the task of logging jsons. An example json is used to show the contents of
    json file shortly.
    ...
    def __init__(self, top_k_labels: int = 1):
        self.frames = {}
        self.video_details = self.video_details = dict(
            frame_width=None, frame_height=None, frame_rate=None, video_name=None
        )
    ...
    def add_frame(self, frame_id: int, timestamp: float | None = None) -> None:
        """
        Args:
            frame_id (int):
    ...
    def find_bbox(self, frame_id: int, bbox_id: int):
        ...
        if not self.bbox_exists(frame_id, bbox_id):
            raise ValueError(f"frame with id: {frame_id} does not contain bbox with id: {bbox_id}")
    ...
    def add_bbox_to_frame(self, frame_id: int, bbox_id: int, top: int, left: int, width: int, height: int) -> None:
        ...
        if self.frame_exists(frame_id):
            ...
            if not self.bbox_exists(frame_id, bbox_id):
                ...
                raise ValueError(f"frame with frame_id: {frame_id} already contains the bbox with id: {bbox_id} ")
        else:
            raise ValueError(f"frame with frame_id: {frame_id} does not exist")
    ...
    def add_label_to_bbox(self, frame_id: int, bbox_id: int, category: str, confidence: float):
        ...
        if not bbox.labels_full(self.top_k_labels):
            ...
            raise ValueError(f"labels in frame_id: {frame_id}, bbox_id: {bbox_id} is fulled")
