...
from smartfridge.utils import YAML, IterableSimpleNamespace
from smartfridge.utils.checks import check_yaml
...
TRACKER_MAP = {"bytetrack": BYTETracker, "botsort": BOTSORT, "ocsort": OCSORT, "hybridsort": HybridSORT}
...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    if cfg.tracker_type not in {"bytetrack", "botsort", "ocsort", "hybridsort"}:
        raise AssertionError(f"Only 'bytetrack', 'botsort', 'ocsort' and 'hybridsort' are supported for now, but got '{cfg.tracker_type}'")
    ...
    if cfg.tracker_type == "botsort" and cfg.with_reid and cfg.model == "auto":
        from smartfridge.nn.modules.head import Detect
