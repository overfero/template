...
from .oc_sort.ocsort import OCSort as OCSort_


class OCSORT(BYTETracker):
    """OC-SORT tracker wrapper for Ultralytics YOLO framework.
    
    An extended version of BYTETracker that uses the OC-SORT (Observation-Centric SORT) algorithm
    for improved object tracking with observation-centric momentum and virtual trajectory proposals.
    
    This class wraps the original OC-SORT implementation to be compatible with Ultralytics YOLO's
    tracking pipeline, converting between Ultralytics Results format and OC-SORT's expected format.
    
    Attributes:
        oc_sort (OCSort_): The underlying OC-SORT tracker instance.
    ...
    Examples:
        Initialize OCSORT and process detections
        >>> tracker = OCSORT(args, frame_rate=30)
        >>> results = yolo_model.detect(image)
        >>> tracked_objects = tracker.update(results, image)
    
    Notes:
        OC-SORT improves upon SORT and ByteTrack by using observation-centric momentum
        to handle non-linear motion and occlusions more effectively.
    """

    def __init__(self, args: Any, frame_rate: int = 30):
        """Initialize OCSORT tracker with configuration parameters.
        ...
        Args:
            args (Any): Configuration namespace containing tracking parameters:
                ...
                - asso_func (str): Association function ('iou', 'giou', 'ciou', 'diou', 'ct_dist')
                - inertia (float): Inertia weight for observation-centric momentum
        ...
        # Initialize the core OC-SORT tracker
        self.oc_sort = OCSort_(
    ...
    def update(self, results, img: np.ndarray | None = None, feats: np.ndarray | None = None) -> np.ndarray:
        ...
        This method converts Ultralytics Results to OC-SORT format, runs the tracking algorithm,
        ...
        Args:
            ...
            feats (np.ndarray, optional): Feature vectors (not used in OC-SORT).
        ...
        # Get image info for OC-SORT - must be plain Python list/tuple with int values
        if img is not None:
            h, w = img.shape[:2]
            img_info = [int(h), int(w)]  # Convert to Python int, not numpy int
            img_size = [int(h), int(w)]  # OC-SORT expects same format
        else:
        ...
        if len(results) == 0:
            # Call OC-SORT update with empty array to maintain tracker state
            self.oc_sort.update(np.empty((0, 5)), img_info, img_size)
            return np.empty((0, 8))
        
        # Convert Ultralytics Results to OC-SORT format
        ...
        # Prepare detections in OC-SORT format: [x1, y1, x2, y2, score]
        ...
        # OC-SORT expects img_info and img_size, returns: [x1, y1, x2, y2, track_id]
        # No try-except for debugging - let errors propagate
        tracks = self.oc_sort.update(detections, img_info, img_size)
        
        # Handle no tracks returned
        if len(tracks) == 0:
            return np.empty((0, 8))
        
        # Convert OC-SORT output to Ultralytics format
        # OC-SORT returns: [x1, y1, x2, y2, track_id]
        # Ultralytics needs: [x1, y1, x2, y2, track_id, score, cls, idx]
        
        # Match tracks to original detections to get scores and classes
        # Create output array
        output_tracks = np.zeros((len(tracks), 8))
        output_tracks[:, :5] = tracks  # [x1, y1, x2, y2, track_id]
        
        # For each track, find the best matching detection to get score and class
        for i, track in enumerate(tracks):
            ...
            if len(ious) > 0:
                ...
                output_tracks[i, 6] = 0    # default class
    ...
    def reset(self):
        """Reset the OC-SORT tracker to its initial state.
        ...
        """
        # Reset OC-SORT internal state
        self.oc_sort.trackers = []
        self.oc_sort.frame_count = 0
        
        # Reset frame counter
        self.frame_id = 0
        
        # Reset track ID counter
        from .oc_sort.ocsort import KalmanBoxTracker
    ...
    def reset_id():
        ...
        Examples:
            >>> OCSORT.reset_id()  # Reset global track ID counter
        """
        from .oc_sort.ocsort import KalmanBoxTracker
        KalmanBoxTracker.count = 0
        BaseTrack.reset_id()
