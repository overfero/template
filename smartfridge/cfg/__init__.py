...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    """
    from smartfridge.solutions.config import SolutionConfig
    ...
    if solution_name == "inference":
        ...
        from smartfridge import solutions
...
def entrypoint(debug: str = "") -> None:
    ...
    if "rtdetr" in stem:  # guess architecture
        from smartfridge import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from smartfridge import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        from smartfridge import SAM
